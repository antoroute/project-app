...
- Le domaine `trust-circle.kavalek.fr` est retenu provisoirement pour les services réseau. Aucun wildcard `*.kavalek.fr` n'est autorisé par cette décision. Ce domaine ne devient ni la marque publique ni la racine définitive des package IDs, et le staging isolé n'est pas exposé publiquement par cette décision.
...
- L'utilisation temporaire de `trust-circle.kavalek.fr` reporte le coût et la migration de domaine, mais impose de conserver les URLs, deep links et configurations remplaçables par environnement.
